...
app.post("/delta", async (req, res) => {
  ...
  if (!changesets || !changesets.length) {
    throw new AppError(
      ...
      "Request body does not contain any delta messages",
      true,
    );
  }
  const statementFilter = (statement: DeltaStatement) =>
    statement.predicate.value === "http://data.europa.eu/m8g/startTime" ||
    statement.predicate.value === "http://data.europa.eu/m8g/endTime" ||
    statement.predicate.value ===
      "https://data.vlaanderen.be/ns/FeitelijkeVerenigingen#heeftErkenning" ||
    statement.predicate.value ===
      "https://data.vlaanderen.be/ns/FeitelijkeVerenigingen#geldigheidsperiode";

  const inserts = changesets
    .map((changeset) => changeset.inserts)
    .flat()
    .filter(statementFilter);
  const deletes = changesets
    .map((changeset) => changeset.deletes)
    .flat()
    .filter(statementFilter);

  const affectedSubjects = [...inserts, ...deletes]
...
app.post("/update-all", async (_req, res) => {
  logger.debug(
    "Updating incorrect/missing/outdates association recognition-statuses",
  );
  res.status(StatusCodes.ACCEPTED).send();
  const now = new Date();
...
CronJob.from({
  ...
  onTick: async () => {
    logger.debug(
      "Running cronjob to update incorrect/missing/outdates association recognition-statuses",
    );
    const now = new Date();
    await updateAssociationStatuses(now);
    logger.debug("Association statuses updated successfully");
