...
Microservice responsible for computing/maintaining the the derived `http://mu.semte.ch/vocabularies/ext/recognitionStatus` predicate on `https://data.vlaanderen.be/ns/FeitelijkeVerenigingen#Vereniging` resources.
The `ext:recognitionStatus` predicate is derived from `fv:Erkenning` resources linked to a `fv:Vereniging`. 
...
The range of `ext:recognitionStatus` is the `http://lblod.data.gift/concept-schemes/c78e079b-5f38-47fd-b284-f22b33172342` concept-scheme (Erkenning status code).
...
The rules for determining the `ext:recognitionStatus` are as follows:
